...
io500_mpirun="mpirun -x LD_PRELOAD=$GKFS"
io500_mpiargs="-n 160 -ppn 16"
io500_mpiargs="-n 320 -npernode 32"
function setup(){
  local workdir="$1"
  local resultdir="$2"
  mkdir -p $workdir $resultdir
  LD_PRELOAD=$GKFS mkdir -p $workdir 
  LD_PRELOAD=$GKFS df -h $workdir
