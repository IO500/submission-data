...
io500_clean_cache="True" # attempt to clean the cache after every benchmark, useful for validating the performance results and for testing with a local node; it uses the io500_clean_cache_cmd (can be overwritten); make sure the user can write to /proc/sys/vm/drop_caches
...
tasks_per_client="32"
function drop_cache_scratch {
	clush -w @clients 'echo 3 > /proc/sys/vm/drop_caches'
}
...
function setup_paths {
  ...
  io500_mpirun="mpirun"
  io500_mpiargs="--oversubscribe --allow-run-as-root -N ${tasks_per_client} --hostfile /root/io500.host"
}

function setup_ior_easy {
  #io500_ior_easy_params="-t 16M -b 10g -F -E --posix.odirect" 
  io500_ior_easy_params="-t 2M -b 75g -F -E" 
#  lfs setstripe -p 32oss $io500_workdir
  mkdir -p $io500_workdir/ior_easy
  /root/create_files5.sh 128 16 ${tasks_per_client} /root/ost_indexes "$io500_workdir/ior_easy/ior_file_easy"
  echo -n ""
}

function setup_mdt_easy {
  io500_mdtest_easy_params="-u -L" # unique dir per thread, files only at leaves
  mkdir -p "$io500_workdir/mdt_easy/#test-dir.0-0"
  /root/create_dirs.sh 128 ${tasks_per_client} "$io500_workdir/mdt_easy/#test-dir.0-0/mdtest_tree"
}

function setup_ior_hard {
  io500_ior_hard_api="POSIX"
  io500_ior_hard_api_specific_options="-E"
  io500_ior_hard_writes_per_proc="$((5000*4))"
  mkdir -p $io500_workdir/ior_hard
  lfs setstripe -c 128 -S 2M $io500_workdir/ior_hard
}
...
function setup_mdt_hard {
  ...
  lfs setdirstripe -c 48 -D "$io500_workdir/mdt_hard"
  lfs setstripe -E 1M -L mdt "$io500_workdir/mdt_hard"
}
