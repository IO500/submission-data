...
thread_cnt=`tail -n 1 $hostfile | sed -e "s/.*=//"`
...
function setup_paths {
  ...
  #io500_mpiargs="-np 2"
  io500_mpiargs="--hostfile ${hostfile} --allow-run-as-root --mca orte_base_help_aggregate 0 singularity.sh -B /usr/lib64 -B /sys/class/infiniband_verbs -B /bin -B /sbin /scratch/centos8.sif"
}
...
function setup_directories {
  ...
  mkdir -p $workdir-scr/ior_easy
  mkdir -p $workdir-app/ior-easy
  mkdir -p $workdir-scr/ior_hard
  mkdir -p $workdir-app/ior-hard
  ../pinfiles_grl.sh $((host_cnt * thread_cnt)) $workdir-scr/ior_easy
  ../pinfiles_grl.sh $((host_cnt * thread_cnt)) $workdir-app/ior-easy
  #mkdir $io500_mdt_easy $workdir-scr/mdt_hard
  #mkdir $io500_mdt_easy $workdir-scr/mdt_easy
  #mkdir $io500_mdt_easy $workdir-app/mdtest-hard
  #mkdir $io500_mdt_easy $workdir-app/mdtest-easy

# lustre extras

  ## Use DNE2 striped directories for mdt_easy
# root can do the next 2 but not a user
  lfs setdirstripe -c -1 $workdir-scr/mdt_easy
  lfs setdirstripe -D -c -1 $workdir-scr/mdt_easy
  lfs setstripe -L mdt -E 1M "$workdir-scr/mdt_easy"
  lfs setdirstripe -c -1 $workdir-app/mdtest-easy
  lfs setdirstripe -D -c -1 $workdir-app/mdtest-easy
  lfs setstripe -L mdt -E 1M "$workdir-app/mdtest-easy"

  # 2 stripes, 16M stripe-size
  #lfs setstripe -p lustrefs.bss017 $io500_ior_easy

  # Use Lustre OST overstriping
  #lfs setstripe -c 48 -o 2,14,26,38,50,62,74,86,98,110,122,134,146,158,170,182,4,16,28,40,52,64,76,88,100,112,124,136,148,160,172,184,6,18,30,42,54,66,78,90,102,114,126,138,150,162,174,186 -S 16M $io500_ior_hard
  #lfs setstripe -c 32 -o 2,14,26,38,50,62,74,86,98,110,122,134,146,158,170,182,4,16,28,40,52,64,76,88,100,112,124,136,148,160,172,184 -S 16M $io500_ior_hard
  #lfs setstripe -c 8 -o 2,26,50,74,98,122,146,170 -S 16M $io500_ior_hard
  lfs setstripe -C 576 -S 16M $workdir-scr/ior_hard
  lfs setstripe -C 576 -S 16M $workdir-app/ior-hard

  # Use DNE2 striped directory for mdt_hard
  lfs setdirstripe -c -1 $workdir-scr/mdt_hard
  lfs setdirstripe -D -c -1 $workdir-scr/mdt_hard
  lfs setstripe -L mdt -E 1M "$workdir-scr/mdt_hard"
  lfs setdirstripe -c -1 $workdir-app/mdtest-hard
  lfs setdirstripe -D -c -1 $workdir-app/mdtest-hard
  lfs setstripe -L mdt -E 1M "$workdir-app/mdtest-hard"
