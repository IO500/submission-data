...
io500_mpirun="mpirun"
io500_mpiargs="--mca coll ^hcoll --oversubscribe -x UCX_TLS=self,sm,rc -x UCX_NET_DEVICES=mlx5_0:1 --allow-run-as-root singularity.sh /exafs -B /work -B /usr -B /sys/class/infiniband_verbs -B /bin -B /sbin -B /etc/libibverbs.d centos8.sif"
#io500_mpiargs="--mca coll ^hcoll --oversubscribe -x UCX_TLS=self,sm,rc -x UCX_NET_DEVICES=mlx5_0:1 --allow-run-as-root"
#io500_mpiargs="-report-bindings -display-devel-map -map-by socket -bind-to socket --allow-run-as-root ./singularity.sh /exafs -B /work -B /usr -B /sys/class/infiniband_verbs -B /bin -B /sbin -B /etc/libibverbs.d centos8.sif "
#io500_mpiargs="--allow-run-as-root ./singularity.sh /exafs -B /work -B /usr -B /sys/class/infiniband_verbs -B /bin -B /sbin -B /etc/libibverbs.d centos8.sif "
...
function setup(){
  ...
  # overstripe
  #lfs setstripe -C 320 $workdir/ior-hard
  lfs setstripe -C 1280 $workdir/ior-hard
  # Metadata OverStriping
  #lfs setdirstripe -D -C 32 $workdir/mdtest-hard
