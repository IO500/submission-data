...
#. /anvme/megware/hnvme/hpcx-v2.22.1-gcc-inbox-redhat9-cuda12-x86_64/hpcx-init.sh
#hpcx_load

eval `tclsh /apps/modules/modulecmd.tcl bash autoinit`
[ -z $MANPATH ] && export MANPATH=:
# add hpc-workspace to path and manpath
export PATH=${PATH}:/apps/hpc-workspace/1.5.0/bin
export MANPATH=${MANPATH}:/apps/hpc-workspace/1.5.0/share/man
ml load gcc/14.2.0
ml load openmpi/5.0.5-gcc14.2.0-cuda
...
io500_mpirun="$(which mpirun) --allow-run-as-root -x LD_LIBRARY_PATH --mca plm_rsh_no_tree_spawn true --mca plm_rsh_num_concurrent 200 --hostfile hostfile"
io500_mpiargs="--map-by ppr:50:package --bind-to core -mca coll_hcoll_enable 0 -x HCOLL_ENABLE_MCAST_ALL=0 -x UCX_NET_DEVICES=mlx5_2:1,mlx5_3:1,mlx5_4:1,mlx5_5:1 -x UCX_LOG_LEVEL=WARN"
...
function setup(){
  ...
  mkdir $workdir/ior-easy $workdir/ior-hard $workdir/ior-rnd1MB $workdir/ior-rnd4K
  mkdir $workdir/mdtest-easy $workdir/mdtest-hard $workdir/mdworkbench
  ...
  if (( $(lfs df $workdir | grep -c MDT) > 1 )); then
    #lfs setdirstripe -D -c -1 $workdir/mdtest-easy
    lfs setdirstripe -D --max-inherit-rr 2 $workdir/mdtest-easy
    lfs setdirstripe -D -c -1 $workdir/mdtest-hard
    #lfs setdirstripe -D -c -1 $workdir/mdworkbench
    lfs setdirstripe -D --max-inherit-rr 5 $workdir/mdworkbench
  fi
  ...
  lfs setstripe -c $osts -S 16M $workdir/ior-rnd1MB
  lfs setstripe -c $osts -S 16M $workdir/ior-rnd4K
  # Try to use DoM if available, otherwise use default for small files
  lfs setstripe -E 1M -L mdt $workdir/mdtest-easy || true #DoM?
  lfs setstripe -E 1M -L mdt $workdir/mdtest-hard || true #DoM?
  lfs setstripe -E 1M -L mdt $workdir/mdworkbench
}
