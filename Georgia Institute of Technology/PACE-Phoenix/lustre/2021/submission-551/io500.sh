...
function setup(){
  ...
  if (( $(lfs df $workdir | grep -c MDT) > 1 )); then
    sudo lfs setdirstripe -D -c -1 $workdir
  fi
  sudo lfs setstripe -c 1 $workdir
  ...
  sudo lfs setstripe -S 8m -C $((osts * 4)) $workdir/ior-easy
  sudo lfs setstripe -S 8m -C $((osts * 4)) $workdir/ior-hard #|| lfs setstripe -c -1 $workdir/ior-hard
  # Try to use DoM if available, otherwise use default for small files
  sudo lfs setstripe -E 64k -L mdt $workdir/mdtest-easy || true #DoM?
  sudo lfs setstripe -E 64k -L mdt $workdir/mdtest-hard || true #DoM?
  sudo lfs setstripe -E 64k -L mdt $workdir/mdtest-rnd
}
