...
io500_mpiargs="-npernode 15 --hostfile hosts"
...
function setup(){
  ...
  # Make the easy test directories in advance, technically violates the rules
  # but allows us to set the xattr to use pinning.
  MAX_MDS=28
  MPI_RANKS=270

  mkdir -p $io500_workdir/mdtest-easy/test-dir.0-0
...
# N-1 RR
  ...
  setfattr -n ceph.dir.pin -v 0 $io500_workdir/mdtest-easy/test-dir.0-0
  MOD=$(( ${MAX_MDS} - ${RR_START} ))
  MAXRANK=$(( ${MPI_RANKS} ))
  ...
  do
    ...
    mkdir -p $io500_workdir/mdtest-easy/test-dir.0-0/mdtest_tree.${RANK}.0
    echo "MPI rank ${RANK} assigned to mds rank ${n}"
    setfattr -n ceph.dir.pin -v ${n} $io500_workdir/mdtest-easy/test-dir.0-0/mdtest_tree.${RANK}.0
  done

  # For the easy test, use cephfs ephemeral pinning
#  sudo setfattr -n ceph.dir.pin.distributed -v 1 $io500_workdir/mdtest-easy/test-dir.0-0
#  sudo setfattr -n ceph.dir.pin.random -v 1.0 $io500_workdir/mdtest-easy

  # For ior-hard, pin to rank 0
  setfattr -n ceph.dir.pin -v 0 $io500_workdir/ior-hard
