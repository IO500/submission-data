#!/bin/bash
#
#SBATCH          --output=log.io500_dfs_rf0.dl360gen12_2eng_10nvme_40tgt.10-32.300sec.%J
#SBATCH           --error=log.io500_dfs_rf0.dl360gen12_2eng_10nvme_40tgt.10-32.300sec.%J
...
mpichversion
echo
echo "DAOS configuration (dl360gen12_2eng_10nvme_40tgt):"
...
# also want cksum:crc32,srv_cksum:on ...
sleep 2
...
#io500_mpirun="mpirun -n 320 $MPI_BIND_TO -genvall -genv D_POOL_WARMUP 1 -genv DAOS_POOL $DAOS_POOL -genv DAOS_CONT $DAOS_CONT -genv DAOS_PREFIX $DAOS_MOUNT"
...
function setup(){
  ...
  io500_mpirun="mpirun -f machinefile.$NODES -n 320 $MPI_BIND_TO -genvall -genv D_POOL_WARMUP 1 -genv DAOS_POOL $DAOS_POOL -genv DAOS_CONT $DAOS_CONT -genv DAOS_PREFIX $workdir "
# io500_mpirun="mpirun -f machinefile.$NODES -n 320 $MPI_BIND_TO -genvall -genv D_POOL_WARMUP 1 -genv LD_PRELOAD /usr/lib64/libpil4dfs.so "
...
cat ${io500_ini}.tmp | sed "s/@STONEWALL/300/g; s/@NPROC/$SLURM_NTASKS/g; s/@DAOS_POOL/$DAOS_POOL/g; s/@DAOS_CONT/$DAOS_CONT/g; s#/tmp/hennecke_pool01/cont_rf0#$DAOS_MOUNT#g" > ${io500_ini}
rm ${io500_ini}.tmp
...
function run_benchmarks {
  echo "$io500_mpirun $io500_mpiargs $HOME/io500-sc25-mpich/io500 $io500_ini $IO500_MODE --timestamp $timestamp"
        $io500_mpirun $io500_mpiargs $HOME/io500-sc25-mpich/io500 $io500_ini $IO500_MODE --timestamp $timestamp
}
