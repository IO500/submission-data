...
io500_mpiargs="-np 1720 --hostfile hostlist.n10"

function setup_dirfrag(){
  local parentdir="$1"
  hard_workdir=/$parentdir/mdtest-hard
  easy_workdir=/$parentdir/mdtest-easy

  oceanfs-set.sh setdirfrag -dirname $hard_workdir 8
  oceanfs-set.sh setdirfrag -dirname $easy_workdir 0
}

function setup(){
  local workdir="$1"
  local resultdir="$2"
  LD_PRELOAD=/opt/fusionstorage/dpc/lib/libpim_preload.so mkdir $workdir $resultdir
  mdhard_dir=$workdir/mdtest-hard
  mdeasy_dir=$workdir/mdtest-easy
  LD_PRELOAD=/opt/fusionstorage/dpc/lib/libpim_preload.so mkdir $mdhard_dir
  LD_PRELOAD=/opt/fusionstorage/dpc/lib/libpim_preload.so mkdir $mdeasy_dir
...
function run_benchmarks {
  SCM_PIM_FLOW_LIMIT=0 LD_PRELOAD=/opt/fusionstorage/dpc/lib/libpim_preload.so $io500_mpirun $io500_mpiargs $PWD/io500 $io500_ini --timestamp $timestamp
}
