...
io500_ini="config.ini"  # You can set the ini file here
io500_mpirun="mpiexec"
#io500_mpiargs="-np 2"

#io500_mpiargs="-np $2"
#io500_mpiargs="--map-by ppr:$2:node:pe=1 --bind-to core --rank-by core --report-bindings"
#io500_mpiargs="--hostfile host.list --map-by ppr:20:node --bind-to core --report-bindings --oversubscribe -x OMPI_MCA_pml=ucx -x UCX_NET_DEVICES=mlx5_0:1 -x UCX_TLS=rc,sm,self -x OMPI_MCA_coll_hcoll_enable=0"
#io500_mpiargs="--hostfile host.list --map-by ppr:${TPN}:node --bind-to core --report-bindings --oversubscribe --mca pml ucx -x UCX_NET_DEVICES=mlx5_0:1 -x UCX_TLS=rc,sm,self -x OMPI_MCA_coll_hcoll_enable=0 -x LD_LIBRARY_PATH=/opt/hpcx/ucx/lib:/opt/hpcx/ompi/lib:$LD_LIBRARY_PATH --prefix /opt/hpcx/ompi"

io500_mpiargs="-f host.list -ppn 20 -bootstrap ssh -bind-to core -genvnone -genv PATH /opt/share/mpich-4.2.2/bin:/u/jlewars/miniforge3/bin:/u/jlewars/miniforge3/condabin:/u/jlewars/.local/bin:/u/jlewars/bin:/usr/share/Modules/bin:/usr/lpp/mmfs/bin:/usr/local/bin:/usr/bin:/usr/local/sbin:/usr/sbin:/opt/dell/srvadmin/sbin:/opt/dell/srvadmin/bin -genv LD_LIBRARY_PATH '' -genv UCX_TLS rc,ud,sm,self,tcp -genv UCX_SOCKADDR_TLS_PRIORITY sockcm -genv UCX_NET_DEVICES mlx5_2:1 -genv UCX_LOG_LEVEL warn -genv UCX_WARN_UNUSED_ENV_VARS n -genv PMI_VERSION 2 -genv MPICH_PMI_TYPE pmi2 -genv HYDRA_PROXY_PORT_RANGE 50000:51000"
