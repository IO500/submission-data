...
export OFS2_ROOT=/home/ofs2
export LD_LIBRARY_PATH=/usr/local/lib64:/usr/local/lib:${OFS2_ROOT}/install:${LD_LIBRARY_PATH}
export LD_LIBRARY_PATH=/usr/mpi/gcc/openmpi-4.1.5rc2/bin:${LD_LIBRARY_PATH}
export PATH=/usr/mpi/gcc/openmpi-4.1.5rc2/bin:$PATH
export OFS2_CFG_PATH=${OFS2_ROOT}/cfg/cfg-c.json
export OFS2_VIEW_PATH=${OFS2_ROOT}/cfg/view.json
export OFS2_PRELOAD=${OFS2_ROOT}/install/libofs2_preload.so
...
io500_mpiargs=" -N 960 \
  ...
  --mca pml ucx \
  --mca routed_radix 960 \
  --mca btl ^vader,tcp,openib,uct \
  --hostfile hostfile \
  -x OFS2_CFG_PATH=${OFS2_CFG_PATH} \
  -x OFS2_VIEW_PATH=${OFS2_VIEW_PATH} \
  -x OFS2_HOOK_FILTER=io500 \
  -x LD_LIBRARY_PATH=${LD_LIBRARY_PATH} \
  -x LD_PRELOAD=/root/c-share/libaccel-io.so:${OFS2_PRELOAD} \
  -x UCX_NET_DEVICES=mlx5_0:1 \
  -x UCX_TLS=ud \
  -x UCX_MAX_RNDV_RAILS=1 \
  -x UCX_IB_MLX5_DEVX=no \
  -x UCX_IB_PREFER_NEAREST_DEVICE=y \
  -x MLX5_SINGLE_THREADED=1 \
  -x UCX_UD_MLX5_RX_QUEUE_LEN=128 \
  -x UCX_UD_VERBS_RX_QUEUE_LEN=128 \
  -x UCX_UD_VERBS_TX_QUEUE_LEN=128 \
  -x UCX_UD_MLX5_TX_QUEUE_LEN=128  \
"
