...
io500_mpiargs="-np 240 --oversubscribe --map-by node --hostfile /lustre/project/nhr-admire/vef/hostfile_mpi -x LIBGKFS_LOG_OUTPUT=/dev/shm/vef_gkfs_client.log -x LIBGKFS_LOG=errors,warnings,info -x LD_PRELOAD=/lustre/project/nhr-admire/vef/gekkofs/build/src/client/libgkfs_intercept.so -x LIBGKFS_HOSTS_FILE=/lustre/project/nhr-admire/vef/run/io500/gkfs_hostfile -x LIBGKFS_PROXY_PID_FILE=/dev/shm/vef_gkfs_proxy.pid taskset -c 0-63"

function setup(){
  local workdir="$1"
  local resultdir="$2"
  mkdir -p $workdir $resultdir
  LIBGKFS_PROXY_PID_FILE=/dev/shm/vef_gkfs_proxy.pid LIBGKFS_HOSTS_FILE=/lustre/project/nhr-admire/vef/run/io500/gkfs_hostfile LD_PRELOAD=/lustre/project/nhr-admire/vef/gekkofs/build/src/client/libgkfs_intercept.so taskset -c 0-63 mkdir -p $workdir/{mdtest-easy,mdtest-hard,ior-easy,ior-hard}
