...
. /root/hpcx-v2.20-gcc-mlnx_ofed-redhat9-cuda12-x86_64/hpcx-init.sh
hpcx_load
...
io500_mpirun="/root/hpcx-v2.20-gcc-mlnx_ofed-redhat9-cuda12-x86_64/ompi/bin/mpirun --bind-to core --hostfile hostfile --allow-run-as-root"
io500_mpiargs="-np 280 --map-by ppr:28:node -x HPCX_HCOLL_DIR -x PKG_CONFIG_PATH -x HPCX_CLUSTERKIT_DIR -x OMPI_HOME -x HPCX_OSU_CUDA_DIR -x HPCX_OSU_DIR -x HPCX_MPI_DIR -x HPCX_OSHMEM_DIR -x HPCX_UCC_DIR -x MPI_HOME -x OSHMEM_HOME -x HPCX_UCX_DIR -x SHMEM_HOME -x LIBRARY_PATH -x HPCX_SHARP_DIR -x HPCX_DIR -x LD_LIBRARY_PATH -x OPAL_PREFIX -x PATH -x HPCX_NCCL_RDMA_SHARP_PLUGIN_DIR -x HPCX_MPI_TESTS_DIR -x CPATH"
...
function setup(){
  ...
  lfs setstripe -C 140 -S 16M $workdir/ior-hard
