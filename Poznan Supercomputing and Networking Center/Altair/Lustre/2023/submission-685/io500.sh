#!/bin/bash
#SBATCH --nodes=2 --ntasks-per-node=6 -p compute -A ku0598
...
#io500_mpiargs="-np 14 --allow-run-as-root"
io500_mpiargs="--allow-run-as-root -np 392 --map-by node --hostfile $HOME/hfile"
...
function setup(){
  ...
  #KW#lfs setstripe -C $((osts * 4)) $workdir/ior-hard ||
  lfs setstripe -C $((osts * 5)) $workdir/ior-hard ||
