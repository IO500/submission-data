...
io500_mpirun="mpirun"
io500_mpiargs="--bind-to core --map-by numa singularity.sh /homel/ddnsihara/pvc -B /usr -B /home --env LD_LIBRARY_PATH=/home/appli/hpcx/2.18.1-gcc-cuda12/ompi/lib rockylinux_9.sif" 
#io500_mpiargs="singularity.sh /homel/ddnsihara/pvc -B /usr -B /home --env LD_LIBRARY_PATH=/home/appli/hpcx/2.18.1-gcc-cuda12/ompi/lib rockylinux_9.sif" 
...
function setup(){
  ...
  for rank in $(seq 0 $((num_process - 1))); do
    dir="$workdir/ior-easy/$rank"
    mkdir -p "$dir"
    ost_index=$((rank % num_osts))
    lfs setstripe -i $ost_index "$dir"
  done

  lfs setstripe -C 2000 $workdir/ior-hard
  sudo lfs setdirstripe -D -c -1 $workdir/mdtest-hard
