...
io500_mpirun="mpirun"
num_mpi="$2"
total_mpi=$((num_mpi * 291))
io500_mpiargs="-x PATH=/apps/mpi/gcc/RHEL9/ucx/1.16/bin:/apps/mpi/gcc/RHEL9/openmpi/4.1.7a1/bin:$PATH -x LD_LIBRARY_PATH=/apps/mpi/gcc/RHEL9/ucx/1.16/lib:/apps/mpi/gcc/RHEL9/openmpi/4.1.7a1/lib:/usr/lib64:$LD_LIBRARY_PATH -np $total_mpi --bind-to numa --mca pml ucx --mca btl ^openib -x UCX_NET_DEVICES=mlx5_1:1,mlx5_4:1 --allow-run-as-root --hostfile /mnt/weka/io500/host_dir/291_hostfile_$num_mpi"
echo "num_mpi : $num_mpi, total_mpi : $total_mpi"
#io500_mpiargs="-x PATH=/apps/mpi/gcc/RHEL9/ucx/1.10/bin:/apps/mpi/gcc/RHEL9/openmpi/4.1.4/bin:$PATH -x LD_LIBRARY_PATH=/apps/mpi/gcc/RHEL9/ucx/1.10/lib:/apps/mpi/gcc/RHEL9/openmpi/4.1.4/lib:/usr/lib64:$LD_LIBRARY_PATH -np 100 --bind-to numa --mca pml ucx -x UCX_NET_DEVICES=mlx5_1:1,mlx5_4:1 --allow-run-as-root --hostfile /mnt/weka/io500/hostfile_2"
