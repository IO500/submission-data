...
io500_mpirun="/opt/ohpc/pub/openmpi-4.1.6/bin/mpiexec"
io500_mpiargs="--mca btl tcp,self --mca routed direct --allow-run-as-root --machinefile machinefile100.txt -npernode 8 --oversubscribe"
...
function setup(){
  ...
  lfs setstripe -c -1 -S 1M $workdir/ior-hard
  lfs setstripe -c 1 $workdir/mdtest-easy
  lfs setstripe -c 1 $workdir/mdtest-hard
  lfs setdirstripe -D -c 1 -i -1 --max-inherit-rr=3 $workdir/mdtest-easy
  lfs setdirstripe -D -c 1 -i -1 --max-inherit-rr=3 $workdir/mdtest-hard
  lfs setstripe -L mdt -E 64K $workdir/mdtest-easy
  lfs setstripe -L mdt -E 64K $workdir/mdtest-hard
