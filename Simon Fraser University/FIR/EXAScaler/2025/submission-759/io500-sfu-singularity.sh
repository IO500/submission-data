...
io500_mpirun="/usr/mpi/gcc/openmpi-4.1.7rc1/bin/mpirun"
# io500_mpiargs="-np 2"
io500_mpiargs="--machinefile machines.new --npernode 128 --oversubscribe --allow-run-as-root -x UCX_LOG_LEVEL=error -x UCX_NET_DEVICES=mlx5_0:1 singularity.sh /firlfs 8 -B /usr /firlfs/ddnsupt/io500/io500/rockylinux_9.sif"
...
####### EXA striping for SFU
echo "workdir is $workdir"
echo "resultdir is $resultdir"

#lfs setstripe -p ddn_ssd $workdir
# IOR
mkdir -p $workdir/ior-easy
mkdir -p $workdir/ior-hard
lfs setstripe -p ddn_ssd $workdir/ior-easy
lfs setstripe -p ddn_ssd $workdir/ior-hard

#lfs setstripe -p ddn_ssd -c -1 -S 1M $workdir/ior-easy
lfs setstripe -C 1024 $workdir/ior-hard          # total PPN

# metadata

mkdir -p $workdir/mdtest-easy
#lfs setdirstripe -D -c 1 $workdir/mdtest-easy
#lfs setstripe -p ddn_ssd $workdir/mdtest-easy
mkdir -p $workdir/mdtest-hard
#lfs setdirstripe -D -c 1 $workdir/mdtest-hard
#lfs setstripe -p ddn_ssd $workdir/mdtest-hard

## changed to RR per SI's note 7/24
 #lfs setdirstripe -D --max-inherit-rr 2 $workdir/mdtest-easy
 #lfs setdirstripe -D -c 1 -i -1 --max-inherit -1 --max-inherit-rr -1 $workdir/mdtest-easy

sudo lfs setdirstripe -D --max-inherit-rr 2 $workdir/mdtest-easy
sudo lfs setdirstripe -D -C 16 $workdir/mdtest-hard
## DoM
 lfs setstripe -E 1m -L mdt $workdir/mdtest-easy
 lfs setstripe -E 1m -L mdt $workdir/mdtest-hard


echo "Lustre striping:"
echo ======================================================
echo mdtest-easy: && lfs getdirstripe $workdir/mdtest-easy
echo mdtest-hard: && lfs getdirstripe $workdir/mdtest-hard
echo ior-easy: && lfs getstripe $workdir/ior-easy
echo ior-hard: && lfs getstripe $workdir/ior-hard
echo ======================================================
