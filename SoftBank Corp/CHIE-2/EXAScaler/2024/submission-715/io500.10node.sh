...
io500_mpirun="mpirun"
io500_mpiargs="--prefix /usr/mpi/gcc/openmpi-4.1.7a1 --mca coll ^hcoll --allow-run-as-root --oversubscribe --map-by ppr:64:socket --mca pml ucx -x UCX_NET_DEVICES=mlx5_0:1 --mca plm_rsh_no_tree_spawn 1 -np 1280 -N 128 -hostfile /lustre/ddn/bin/hostfile.10node /lustre/ddn/bin/singularity.sh /lustre  -B /usr -B /sys/class/infiniband_verbs -B /bin -B /sbin -B /etc/libibverbs.d -B /tmp /lustre/ddn/bin/ubuntu22.04.sif " 
...
function setup(){
  ...
  mkdir $workdir/mdtest-easy $workdir/mdtest-hard
  #lfs setstripe -c -1 $workdir/ior-hard
  lfs setstripe -C 40 $workdir/ior-hard
  #lfs setdirstripe -D -C 32 $workdir/mdtest-hard
  lfs setdirstripe -D -c -1 -i -1 --max-inherit=-1 $workdir/mdtest-hard
