...
#SBATCH --reservation=dac

module purge
module load intel/bundles/complib/2019.0 slurm rhel7/global
ROOT=$PWD
workdir=/dac/fs1
...
function setup_directories {
  ...
  if [ -d ${io500_workdir}/mdt_easy ]; then
     mv ${io500_workdir}/mdt_easy ${io500_workdir}/mdt_easy_${timestamp}
  fi
...
function setup_ior_easy {
  ...
  io500_ior_easy_size=18 #20 #12
  # 2M writes, 2 GB per proc, file per proc
  io500_ior_easy_params="-a POSIX -B -t 1m -b ${io500_ior_easy_size}g -F"
}

function setup_mdt_easy {
  io500_mdtest_easy_params="-u -L" # unique dir per thread, files only at leaves
  io500_mdtest_easy_files_per_proc=4200 #3000
}

function setup_ior_hard {
  io500_ior_hard_writes_per_proc=20500 #9000
  io500_ior_hard_other_options="-a POSIX" #e.g., -E to keep precreated files using lfs setstripe, or -a MPIIO
}

function setup_mdt_hard {
  io500_mdtest_hard_files_per_proc=29000 #3000
...
io500_info_filesystem_version='2.11'
io500_info_filesystem_vendor='xxx'
# client side info
io500_info_num_client_nodes='528'
io500_info_procs_per_node='8'
# server side info
io500_info_num_metadata_server_nodes='20'
