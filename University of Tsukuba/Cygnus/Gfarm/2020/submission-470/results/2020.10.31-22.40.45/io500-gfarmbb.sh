...
#PBS -b 10
#PBS -T openmpi
#PBS -v NQSV_MPI_VER=4.0.3/gcc8.3.1-cuda10.2
...
export ROMIO_FSTYPE_FORCE=gfarms: # for ior-hard
TMPDIR=/scr/$USER
DIR=/tmp/gfarm-$USER

eval `gfarmbb -h $PBS_NODEFILE -l -scr $TMPDIR -m $DIR start`
gfarmbb -h $PBS_NODEFILE -l -scr $TMPDIR -m $DIR status
gfstatus
gfdf
gfhost -lv
df $DIR
...
function setup_paths {
  ...
  io500_mpirun="mpirun"
  io500_mpiargs="-f $PBS_NODEFILE -envlist GFARM_CONFIG_FILE,LD_LIBRARY_PATH,ROMIO_FSTYPE_FORCE -np 20 -map-by socket:1 -bind-to socket"
}
...
gfrm -rf $io500_workdir
#mkdir -p $PBS_O_WORKDIR/results
#cp -a $io500_result_dir $PBS_O_WORKDIR/results
gfarmbb -h $PBS_NODEFILE -l -scr $TMPDIR -m $DIR stop
